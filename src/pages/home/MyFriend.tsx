...
interface MyFriendsInterface {
  navigation: any
}

const MyFriends: FC<MyFriendsInterface> = ({ navigation }) => {
  ...
  const searchFriends = async (keywords = "") => {
    ...
    try {
      ...
      setListContactData(data.data)
      setTotalFriend(data.total)
    } catch (error) {
      ToastAndroid.show("Gagal menampilkan daftar kontak !", ToastAndroid.SHORT)
    } finally {
      setLoading(false)
  ...
  return (
    <View className="flex-1 p-4 bg-white">
      {/* <TouchableOpacity onPress={() => navigation.navigate("ConfirmFriends")} className="flex-row items-center">
                <View className="w-2/12">
                    <Assets.IconMyfriedns width={45} height={45} />
                </View>
                <View className="w-8/12">
                    <Text className="font-satoshi text-black font-medium text-md">
                        10 Teman Anda Menunggu Konfirmasi Pertemanan!
                    </Text>
                </View>
