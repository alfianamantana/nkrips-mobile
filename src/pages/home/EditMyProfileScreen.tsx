...
const EditProfile: FC<editProfileInterface> = ({ navigation, route }) => {
  ...
  const detailProfile = async () => {
    try {
      ...
      setNamaLengkap(profile.user.name)
      setTanggaLahir(profile.user.date_of_birth)
      setUrlProfile(profile.user.profile_picture_url ? profile.user.profile_picture_url : "")
    } catch (error) {
  ...
  const uploadImage = async (image: any) => {
    try {
      const fileUri = Platform.OS === 'android' ? image.path : image.path.replace('file://', '');
      const fileType = image.mime;

      const postImg = await uploadFile(fileUri, fileType);
