...
const ModalsKabupaten: FC<ModalsKabupatenInterface> = ({ isShow, handleShowHideModals, handleSelected, idProvinsi }) => {
    const [keywords, setKeywords] = useState("")
    const [activeData, setActiveData] = useState({ value: 0, label: "" })
    const [loading, setLoading] = useState(true)
    const [listData, setData] = useState<Schema.GetResponseAdmProvinsiIdProvinsiKotaKab>([])

    const getData = async (search = "") => {
        setLoading(true)
        try {
            const dataResponse = await kabRequest(idProvinsi)
            if (search === "") {
...
export default ModalsKabupaten